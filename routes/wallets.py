...
from utils import is_valid_address, is_supported_chain
...
def verify_wallet(wallet: WalletCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if not is_supported_chain(wallet.chain):
        raise HTTPException(status_code=400, detail="Unsupported or invalid chain.")

    return {"message": "Wallet address and chain are valid."}
...
def add_wallet(
    ...
    db_wallet = Wallet(
        address=wallet.address, chain=wallet.chain, user_id=current_user.id
    )
    db.add(db_wallet)
    db.commit()
